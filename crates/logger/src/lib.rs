...
impl<'a> Logger<'a> {
    ...
    pub fn trace(&self, message: &str) -> &Self {
        if self.debug_level > 0 {
            self.logger.lock().expect("Failed to lock logger").log(message);
        }
        self
    }
    ...
    pub fn info(&self, message: &str) -> &Self {
        self.logger.lock().expect("Failed to lock logger").info(message);
        self
    }

    /// Logs a warning message.
    pub fn warn(&self, message: &str) -> &Self {
        self.logger.lock().expect("Failed to lock logger").warn(message);
        self
    }

    /// Logs an error message.
    pub fn error(&self, message: &str) -> &Self {
        self.logger.lock().expect("Failed to lock logger").error(message);
        self
    }

    /// Logs a success message.
    pub fn success(&self, message: &str) -> &Self {
        self.logger.lock().expect("Failed to lock logger").success(message);
        self
    }

    /// Logs a newline.
    pub fn newline(&self, count: usize) -> &Self {
        self.logger.lock().expect("Failed to lock logger").newline(count);
        self
    }

    /// Indents the logger.
    pub fn indent(&self, count: usize) -> &Self {
        self.logger.lock().expect("Failed to lock logger").indent(count);
        self
    }
    ...
    pub fn add_style(&self, name: &str, styles: Vec<&'a str>) -> &Self {
        self.logger.lock().expect("Failed to lock logger").add_style(name, styles);
        self
    }

    /// Logs a loading message with a spinner.
    pub fn loading(&self, message: &str) -> &Self {
        self.logger.lock().expect("Failed to lock logger").loading(message);
        self
    }
